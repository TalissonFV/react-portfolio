...
const Projetos = () => {
  ...
  return (
    <section id="projetos" className="section-spacing bg-beige">
      <div className="container mx-auto px-4 card-style bg-[#f09d51]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projetos.map((projeto, index) => (
            <div
              key={index}
              className="card-style hover:shadow-xl transition-shadow flex flex-col"
            >
              {projeto.imagem && (
                <img
                  src={projeto.imagem}
              ...
              <div className="p-6 flex flex-grow flex-col">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {projeto.tecnologias.map((tech, i) => (
                    <span
                      key={i}
                      className="px-3 py-1 rounded-full text-sm bg-light text-dark"
                    >
                      {tech}
                    </span>
                  ))}
